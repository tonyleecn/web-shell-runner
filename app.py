...
def add_script():
    ...
    if form.validate_on_submit():
        ...
        max_position = db.session.query(
            db.func.max(Script.position)).scalar() or 0
...
def run_script(id, operation):
    ...
    # 运行脚本
    try:
        process, _, pid = ScriptRunner.run_script(
            script_path, user_param, action_param)
        return jsonify({
            'status': 'success',
        ...
        try:
            ...
            for script_data in data['scripts']:
                ...
                if existing_script:
                    ...
                    existing_script.description = script_data.get(
                        'description', '')
                    ...
                    # 脚本参数
                    existing_script.start_user_param = script_data.get(
                        'start_user_param', '')
                    existing_script.start_action_param = script_data.get(
                        'start_action_param', '')
                    existing_script.stop_user_param = script_data.get(
                        'stop_user_param', '')
                    existing_script.stop_action_param = script_data.get(
                        'stop_action_param', '')
                    existing_script.restart_user_param = script_data.get(
                        'restart_user_param', '')
                    existing_script.restart_action_param = script_data.get(
                        'restart_action_param', '')
                    existing_script.deploy_user_param = script_data.get(
                        'deploy_user_param', '')
                    existing_script.deploy_action_param = script_data.get(
                        'deploy_action_param', '')
                else:
                    # 创建新脚本
                    script = Script(
                        ...
                        # 脚本参数
                        start_user_param=script_data.get(
                            'start_user_param', ''),
                        start_action_param=script_data.get(
                            'start_action_param', ''),
                        stop_user_param=script_data.get('stop_user_param', ''),
                        stop_action_param=script_data.get(
                            'stop_action_param', ''),
                        restart_user_param=script_data.get(
                            'restart_user_param', ''),
                        restart_action_param=script_data.get(
                            'restart_action_param', ''),
                        deploy_user_param=script_data.get(
                            'deploy_user_param', ''),
                        deploy_action_param=script_data.get(
                            'deploy_action_param', '')
                    )
                    db.session.add(script)
...
def script_content():
    ...
    try:
        ...
        if not script_path or not os.path.isfile(script_path):
            return jsonify({'status': 'error', 'message': f'无效的脚本路径: {script_path}'}), 400
        ...
        return jsonify({'status': 'error', 'message': f'读取脚本内容出错: {str(e)}'}), 500
